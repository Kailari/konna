...
public class RestartGameSystem implements EcsSystem<RestartGameSystem.Resources, EcsSystem.NoEntities, RestartGameSystem.EventData> {
    ...
    public void tick(
        ...
        if (inputs.inputRestart) {
            resources.stateEvents.fire(new StateEvent.ChangeMode(GameplayGameMode.create(System.nanoTime(),
                                                                                         resources.timeManager)));
        } else if (inputs.inputMenu) {
            resources.stateEvents.fire(new StateEvent.ChangeMode(MainMenuGameMode.create()));
        }
    }

    public static record Resources(Inputs inputs, StateEvents stateEvents, TimeManager timeManager) {}
